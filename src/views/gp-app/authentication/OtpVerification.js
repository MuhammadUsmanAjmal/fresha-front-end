...
const OtpVerification = () => {
  ...
  const successForget = forgetPass?.userData?.success;
  const otpVerify = useSelector((state) => state.otp);
  const successOTP = otpVerify?.otpVerifying?.success;
  const errorOTP = otpVerify?.otpVerifying?.error;
  ...
  useEffect(() => {
    if (errorOTP) {
      toast.error(<ToastContent error={errorOTP} />, {
        transition: Slide,
  ...
  useEffect(() => {
    if (successOTP) {
      // toast.success(<ToastContent success={submitOTP} />, {
      //   transition: Slide,
      //   hideProgressBar: true,
      //   autoClose: 2000,
      // });
  ...
  if (!isUserLoggedIn() && successForget) {
    return (
