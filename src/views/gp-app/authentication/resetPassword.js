...
const Reset = (props) => {
  ...
  const successOTP = otpVerify?.otpVerifying?.success;
  const resetPassword = useSelector((state) => state.changePassword);
  const success = resetPassword?.password?.success;
  const error = resetPassword?.password?.error;
  ...
  useEffect(() => {
    if (success) {
      reset();
      // toast.success(
      //   <ToastContent success={resetPassword?.password?.message} />,
      //   {
      //     transition: Slide,
  ...
  useEffect(() => {
    if (error) {
      toast.error(<ToastContent error={resetPassword?.password?.error} />, {
        transition: Slide,
        hideProgressBar: true,
        autoClose: 2000,
      });
  ...
  const onSubmit = (data) => {
    if (data.password === data.confirmPassword) {
      ...
      toast.error(<ToastContent error={"Password not match"} />, {
        transition: Slide,
        hideProgressBar: true,
        autoClose: 2000,
      });
