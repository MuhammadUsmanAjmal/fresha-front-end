...
const PaymentSelection = ({ viewChange, orderID }) => {
  ...
  const { loading } = Mode;
  const Information = JSON.parse(localStorage.getItem("userData"));
  const SaloonId = Information?.data?.salonId;
  const BranchId = Information?.data?.branchId;

  useEffect(() => {
    dispatch(paymentModeAction(SaloonId));
  }, []);
  useEffect(() => {
    if (OrderSuccess) {
      MySwal.fire({
